                ...
                if ((status[0] & 0x20) == 0x20)    //判断是否缺纸
                    ...
                    for (int i = 0; i < nCount; i++) {
                        ...
                        if (nPrintContent >= 1) {
                            pos.POS_Reset();
                            pos.POS_FeedLine();
                            pos.POS_S_SetBarcode(pbody.sell_code, 0 + line_offset, 69, 2, 50, 0, 0);
//                            pos.POS_TextOut(pbody.sell_code + "\r\n", 0, line_offset, 1, 1, 0, 0);
...
//                            pos.POS_TextOut("\r\n", 0, 0, 0, 0, 0, 0);
                            ...
                            for (GoodsBean bean : pbody.list) {
                                String name = bean.name.replace(" ", "");
                                pos.POS_TextOut(name.length() > 10 ? name.substring(0, 10) + ("\r\n") + name.substring(10) : name, 0, 0 + line_offset, 0, 0, 0, 0);
