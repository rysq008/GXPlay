...
//                            }.getType())
                            ...
                            .map((Function<ResponseBody, HttpResultModel>) responseBody -> {
                                ...
                                if (map.get("resultCode").equals("1")) {
                                    HttpResultModel<List<SkuTableEntiry>> list = gson.fromJson(result, new TypeToken<HttpResultModel<List<SkuTableEntiry>>>() {
                                    }.getType());
                                    for (SkuTableEntiry tableEntiry : list.resultData) {
                                        GoodsBean goodsBean = new GoodsBean();
                                    ...
                                    skuTableEntiry.put(list.resultData);
                                    resultModel.resultCode = 1;
                                }
                                resultModel.resultContent = map.get("resultContent").toString();
                                return resultModel;
                            })
                            ;
                });
