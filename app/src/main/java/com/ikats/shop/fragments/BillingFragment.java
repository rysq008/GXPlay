...
//        MqttAndroidClient mqttAndroidClient = new MqttAndroidClient(App.getApp(), "tcp://223.70.196.70:11616", "clientId");
//        //设置回调
...
//                Log.e("TAG", "connectionLost: -->"+token );
//            }
//        });
//        //相关配置:
...
//            });
//            //主题订阅
//            mqttAndroidClient.subscribe("TEST", 1);
//
////        然后就可以在callback,messageArrived方法里收到服务端发来的消息.发送消息
    ...
    public void onViewClick(View view) {
        switch (view.getId()) {
            case R.id.billing_enter_submit_btn:
                ...
                if (Kits.Empty.check(cur_product_id) && Kits.Empty.check(enter_url_et.getHint())) {
                ...
                Flowable<HttpResultModel<OrderResultBean>> pay = DataService.builder().buildReqUrl(App.getSettingBean().shop_url + "api/create")
                        .builderRequestBody(OrderinfoRequestBody.newBuilder().setCurrentUser("mobile", verifyResultBean.mobile)
                                ...
                                .setCode(promo_code)
                                .setReceiver(verifyResultBean.name, App.getSettingBean().shop_area, App.getSettingBean().shop_address, "", verifyResultBean.mobile).builder())
            ...
            case R.id.billing_verify_by_wechat_iv:
                ...
                RxLoadingUtils.subscribeWithDialog(context, wechat, bindToLifecycle(), httpResultModel -> {
                    ...
                    DialogFragmentHelper.builder(context -> {
                        ...
                        return new AlertDialog.Builder(context).setTitle("提示").setView(imageView).setNegativeButton("取消", null)
                                .setPositiveButton("确定", (dialog, which) -> dialog.cancel()).create();
                    }, true).show(getChildFragmentManager(), "");
                });
                break;
