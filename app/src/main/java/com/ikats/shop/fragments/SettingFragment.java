...
import com.flvextractorcompat.FlvExtractorCompat;
...
public class SettingFragment extends XBaseFragment {
    ...
    Spinner area_sp;
    @BindView(R.id.setting_shop_zipcode_et)
    EditText zipcode_et;
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        province_sp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (settingBean.province_index != position) {
                    settingBean.city_index = 0;//settingBean.city_index < city_adp.getSize() ? settingBean.city_index : city_adp.getSize() - 1;
                    settingBean.area_index = 0;
                }
        ...
        city_sp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (settingBean.city_index != position) {
                    settingBean.area_index = 0;
                }
    ...
    @OnClick({R.id.setting_send_by_self_cb, R.id.setting_send_by_express_cb, R.id.setting_video_preview_open_cb,
            ...
            R.id.setting_connect_preview_test_tv, R.id.setting_connect_record_test_tv, R.id.setting_update_btn})
...
//                });
                ...
                Flowable<HttpResultModel> flowable =
                        DataService.builder().buildReqUrl(String.format(record_url.concat("/record?sellNo=%s&channel=%s"), video_name, settingBean.record_channel))
                                ...
                                .request(ApiService.HttpMethod.GET).flatMap((Function<ResponseBody, Flowable<HttpResultModel>>) responseBody -> {
                ...
                RxLoadingUtils.subscribeWithDialog(context, flowable, bindToLifecycle(), httpResultModel -> {
                    ...
                    mediaSources = new ProgressiveMediaSource.Factory(dataSourceFactory, FlvExtractorCompat.FACTORY).createMediaSource(suri);
                    // 将播放器附加到view
            ...
            case R.id.setting_submit_btn:
                DialogFragmentHelper.builder(context -> new AlertDialog.Builder(context, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT).setTitle("提示").setMessage("是否保存本页面配置").setNegativeButton("取消", null).setPositiveButton("确定", (dialog, which) -> {
                    ...
                    settingBean.zipCode = zipcode_et.getText().toString().trim();
            ...
            case R.id.setting_update_btn:
                        ...
                        DataService.builder().buildReqUrl(App.getSettingBean().manage_url + "login/getToken")
                            ...
                            return DataService.builder().buildReqUrl(App.getSettingBean().manage_url + "ownersku/queryshopsku")
                                    ...
                                    .doOnNext((Consumer<HttpResultModel<List<SkuTableEntiry>>>) listHttpResultModel -> {
                                        Box skuTableEntiry = App.getBoxStore().boxFor(SkuTableEntiry.class);
                                        ...
                                        if (listHttpResultModel.isSucceful()) {
                                            for (SkuTableEntiry tableEntiry : listHttpResultModel.resultData) {
                                                ...
                                                App.products.put(goodsBean.barcode, goodsBean);
                                            }
                ...
                RxLoadingUtils.subscribeWithDialog(context, f_token, bindToLifecycle(), httpResultModel -> {
                    if (httpResultModel.isSucceful()) {
                        ToastUtils.showLong("fetch sku data success !");
                    } else {
                        ToastUtils.showLong(httpResultModel.resultContent);
                    }
                }, netError -> ToastUtils.showLong(netError.getMessage()), false);
                break;
        }
    }
    ...
    public void setExpressShow(boolean send_by_express) {
        ...
        zipcode_et.setVisibility(send_by_express ? View.GONE : View.VISIBLE);
        shop_name_et.setVisibility(send_by_express ? View.GONE : View.VISIBLE);
        shop_code_et.setVisibility(send_by_express ? View.GONE : View.VISIBLE);
        shop_cashier_et.setVisibility(send_by_express ? View.GONE : View.VISIBLE);
    }
