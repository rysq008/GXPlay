...
//        });
    ...
    public boolean addOrupdateData(GoodsBean bean) {
...
//            notifyItemChanged(index, 0);
            ...
            if (data.size() == 0 || bean.shopcode.equals(data.get(0).shopcode)) {
                bean.count++;
//            data.add(bean);
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setDisplay(BillingItemAdapter adapter, GoodsBean itemType, int position) {
            View.OnClickListener onClickListener = v -> {
                if (adapter.getRecItemClick() != null) {
                    adapter.getRecItemClick().onItemClick(position, itemType, TAG_VIEW, ViewHolder.this);
                }
            };
...
//            itemType.pos_in_list = position;
            ...
            delIv.setOnClickListener(v -> {
                itemType.count = 0;
                itemType.amount = 0;
                adapter.removeElement(itemType);
            });
            goodsTv.setOnClickListener(onClickListener);
            codeTv.setOnClickListener(onClickListener);
            inputEt.setOnClickListener(onClickListener);
            priceTv.setOnClickListener(onClickListener);
            sellTv.setOnClickListener(onClickListener);
