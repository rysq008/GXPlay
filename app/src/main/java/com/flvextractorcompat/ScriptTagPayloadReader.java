 ...
 */
package com.flvextractorcompat;
...
/* package */ final class ScriptTagPayloadReader extends TagPayloadReader {
  ...
  private static final String KEY_HAS_KEYFRAMES = "hasKeyframes";
  ...
  private FlvSeekMap seekMap;

  @Nullable
  public FlvSeekMap getSeekMap() {
    return seekMap;
  }
  ...
  protected boolean parsePayload(ParsableByteArray data, long timeUs) throws ParserException {
    ...
    //有hasKeyframes域，且hasKeyframes的值为true
    if (metadata.containsKey(KEY_HAS_KEYFRAMES) && (boolean) metadata.get(KEY_HAS_KEYFRAMES)) {
      Log.i("ScriptReader", "hasKeyframes = true");
      //从元数据中取keyframes数据
      final HashMap<String, ArrayList<Double>> keyframes = (HashMap) metadata.get(KEY_KEYFRAMES);
      //keyframes不为null，且filepositions、times字段均存在
      if (keyframes != null && keyframes.containsKey(KEY_FILE_POSITIONS) && keyframes
          .containsKey(KEY_TIMES)) {
        //取filepositions集合
        ArrayList<Double> filePositionArray = keyframes.get(KEY_FILE_POSITIONS);
        //取times集合
        ArrayList<Double> timesArray = keyframes.get(KEY_TIMES);
        //只有在filepositions、times集合都不为空且长度相等时，在已进行seek
        if (filePositionArray != null && filePositionArray.size() > 0 && timesArray != null
            && timesArray.size() == filePositionArray.size()) {
          seekMap = new FlvSeekMap(durationUs, timesArray, filePositionArray);
        }
        Log.i("ScriptReader", "filePositions = " + filePositionArray);
        Log.i("ScriptReader", "times = " + timesArray);
      }
    }
    return false;
  }
