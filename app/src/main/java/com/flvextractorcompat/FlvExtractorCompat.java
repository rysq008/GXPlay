 ...
 */
public final class FlvExtractorCompat implements Extractor {

  /**
   * Factory for {@link FlvExtractorCompat} instances.
   */
  public static final ExtractorsFactory FACTORY = () -> new Extractor[]{new FlvExtractorCompat()};
  ...
  private void log(String message) {
    Log.d("FlvExtractor", message);
  }

  @Override
  public void seek(long position, long timeUs) {
    log("seek(" + position + "," + timeUs + ")");
    if (state > STATE_READING_FLV_HEADER) {
  ...
  public int read(ExtractorInput input, PositionHolder seekPosition) throws IOException,
    ...
    log("read(" + input + "," + seekPosition.position + ")");
    while (true) {
      switch (state) {
...
//      ensureReadyForMediaOutput();
      ...
      FlvSeekMap seekMap = metadataReader.getSeekMap();
      if (seekMap != null) {
