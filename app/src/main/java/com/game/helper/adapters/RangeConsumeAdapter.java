 ...
 * 收益排行榜adapter
 */
public class RangeConsumeAdapter extends SimpleRecAdapter<ItemType, RangeConsumeAdapter.PhotoViewHolder> {

    private Activity mActivity;


    public RangeConsumeAdapter(Activity context) {
        super(context);
        mActivity = context;
    }
    ...
    public PhotoViewHolder newViewHolder(View itemView) {
        return new RangeConsumeAdapter.PhotoViewHolder(itemView);
    }
    ...
    public int getLayoutId() {
        return R.layout.item_range_consume;
    }
...
//                Glide.with(activity).load(data.getMember().getIcon()).into(ivAvatar.getAvatarView());
            ...
            //收益金币
            incomeCoin.setText(data.getConsume_amount());
